  ...
  return (
    <>
      <motion.article
        ...
        role="article"
        aria-label={`角色卡片：${character.nickname}`}
      >
        <div className="flex items-center gap-5">
          <div 
            className="relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl bg-slate-100 ring-4 ring-slate-50 dark:bg-slate-800 dark:ring-slate-800/50"
            role="img"
            aria-label={`${character.nickname}的头像`}
          >
            {character.avatarUrl ? (
              <img
                src={character.avatarUrl}
                alt={`${character.nickname}的头像`}
        ...
        <div className="mt-8 grid grid-cols-2 gap-3">
          <Link
            ...
            aria-label={`查看${character.nickname}的画像分析`}
          >
          ...
          <Link
            ...
            aria-label={character.analysisStatus === "DONE" ? `与${character.nickname}聊天` : "请先完成人物分析"}
            aria-disabled={character.analysisStatus !== "DONE"}
          >
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 border-t border-slate-100 pt-5 dark:border-slate-800/50">
          <Link
            ...
            aria-label={`编辑${character.nickname}的资料`}
          >
          ...
          <button
            ...
            aria-label={`删除角色${character.nickname}`}
          >
