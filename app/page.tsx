  ...
  return (
    <main className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-8 px-4 py-10 sm:px-6 lg:px-8">
      ...
      >
        ...
        <div className="absolute -bottom-24 -left-24 h-96 w-96 rounded-full bg-cyan-600/10 blur-[100px]" />
      ...
      <section className="space-y-6">
        <div className="flex items-end justify-between px-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">数字化身库</h2>
        ...
        {isLoading ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-64 animate-pulse rounded-[2rem] bg-slate-100 dark:bg-slate-800" />
            ))}
          </div>
          ...
          >
            ...
            <Link href="/characters/new" className="mt-8 flex items-center gap-2 text-sm font-black text-blue-600 hover:text-blue-700">
              去创建一个 <ArrowRight className="h-4 w-4" />
            </Link>
          </motion.div>
        ) : (
          <motion.div 
